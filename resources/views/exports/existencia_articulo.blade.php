...
<table border="1" class="reporte">
    ...
    <tbody>
        <tr>
            <td colspan="2"> {{ $invoices->articulo }} </td>
            <td colspan="2"> {{ $invoices->origenLabel }} </td>
            <td colspan="2"> {{ $invoices->productos->reduce( function ($sum, $item) { return $sum + $item->cantidad; }, 0) }} </td>
        </tr>
        <tr>
        ...
        @foreach( $invoices->productos as $prod )
            <tr>
                <td> {{ $invoices->articulo }} </td>

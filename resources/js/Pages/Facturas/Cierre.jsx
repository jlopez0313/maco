...
export const Cierre = ({ auth, setIsOpen }) => {
    ...
    const onGetReporte = async () => {
        ...
        const sum = data
            .filter((item) => item.forma_pago?.id == 1)
            .map((item) => {
        ...
        const credito = data
            .filter((item) => item.forma_pago?.id == 2)
            .map((item) => {
                item.detalles.forEach((_item) => {
                    let impuestos = 0;
        ...
        if (credito.length) {
            const total = credito.reduce((x, acum) => {
                return x + acum;
            });
            setTotalCredito(toCurrency(total));
        }
    };
            ...
            <div className="max-w-6xl mx-auto sm:px-6 lg:px-8">
                <div className="grid grid-cols-1 gap-4">
                    ...
                    <div>
                        <InputLabel
                            htmlFor="documento"
                            value="Total de Ventas a Crédito del día"
                        />
                        <TextSpan
                            className="mt-1 block w-full"
                            value={totalCredito}

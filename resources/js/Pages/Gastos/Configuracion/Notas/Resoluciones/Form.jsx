...
export default ({ id, empresasId, estados, auth, setIsOpen, onReload }) => {
    ...
    const submit = async (e) => {
        ...
        try {
            if ( id ) {
                await axios.put(`/api/v1/permisos/${id}`, data);
            } else {
                await axios.post(`/api/v1/permisos`, data);
            }
        } catch (e) {
            console.error( e );
        }
    ...
    const onGetItem = async () => {
        ...
        const { data } = await axios.get(`/api/v1/permisos/${id}`);
    ...
    const onGetConsecutivo = async () => {
        const { data: info } = await axios.get(`/api/v1/permisos/consecutivo/${empresasId}`);
